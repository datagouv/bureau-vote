"""
Various cleaning methods. Some can be applied directly on the input addresses table, while others must be applied on addresses 
that have been previously geocoded with the "geo" module. 
"""
    ...
    if "chez" in x:
        ...
        if len(to_parse) > 1:
            if to_parse[1] in [
                ...
                if len(to_parse) > 4:
                    if to_parse[4] == "et":
                        if len(to_parse) > 5 and to_parse[5] in [
                            "m.",
                            "m",
                            "mr",
                            "mme",
...
def clean_geocoded_types(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean some dtypes of the dataframe after the geocoding step
...
def clean_failed_geocoding(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    assert (
        "result_score" in df.columns
        and "result_postcode" in df.columns
        and "CP" in df.columns
        and "CP_BV" in df.columns
        and "result_citycode" in df.columns
        and "Code communeRéférentiel" in df.columns
    ), "the dataframe does not include required columns for cleaning"
    ...
    return df[
        (df.result_score > 0.5)
        & (df.result_citycode == df["Code communeRéférentiel"])
        & (df.result_postcode == df.CP)
    ].dropna(subset=["CP", "result_citycode", "result_postcode"])
